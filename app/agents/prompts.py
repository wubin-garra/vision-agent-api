...
BASE_SYSTEM = f"""你是视觉智能体，帮助用户理解照片中的内容。
...
AGENT_PROMPTS: dict[AgentId, str] = {
    AgentId.LOCAL_GUIDE: BASE_SYSTEM
    + """
角色：本地向导 (local_guide)
...
""",
    AgentId.FOOD_SCAN: f"""你是 Chance 风格的「食识拍」营养分析智能体 (food_scan)，扫描餐食并估算营养信息。
输出必须是合法 JSON，严格遵循以下 schema，不要输出 markdown 代码块：
{FOOD_SCAN_INSIGHT_JSON_SCHEMA}
...
""",
    AgentId.TEXT_READER: BASE_SYSTEM
    + """
...
""",
    AgentId.GENERAL_CURIOSITY: BASE_SYSTEM
    + """
...
ROUTER_SYSTEM = """你是视觉场景分类器。根据图片选择最合适的专项智能体。
输出合法 JSON：
{
...
FOLLOWUP_SYSTEM = """你是视觉智能体助手。用户已看到对某张照片的分析，现在追问。
...
FOOD_SCAN_FOLLOWUP_SYSTEM = f"""你是「食识拍」营养顾问，风格参考 Chance AI：专业、温和、有画面感。
...
SCENE_TO_AGENT: dict[str, AgentId] = {
...
FOLLOWUP_CHIPS: dict[AgentId, list[str]] = {
...
def resolve_followup_chips(insight) -> list[str]:
    """优先使用 insight 内的 explore_chips，否则回退到默认 chips。"""
    chips = getattr(insight, "explore_chips", None)
    if chips:
        combined = list(chips.culinary) + list(chips.nearby)
        if combined:
            return combined[:6]
    agent_id = insight.agent_id if hasattr(insight, "agent_id") else AgentId.GENERAL_CURIOSITY
    return FOLLOWUP_CHIPS.get(agent_id, [])


def flatten_followup_suggestions(result: dict) -> list[str]:
    """从结构化追问结果中提取扁平建议列表。"""
    flat = result.get("suggested_followups") or []
    if flat:
        return flat[:6]

    structured = result.get("structured_answer") or {}
    groups = structured.get("suggestion_groups") or []
    questions: list[str] = []
    for group in groups:
