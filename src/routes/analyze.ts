...
import { resolveFollowupChips } from "../agents/prompts.js";
import {
  AgentId,
...
export async function analyzeRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/followup", async (request, reply) => {
    ...
    memoryRepository.appendFollowup(body.memory_id, body.question, answer);

    return {
      memory_id: body.memory_id,
      answer,
      suggested_followups: (result.suggested_followups as string[]) ?? [],
    };
  });
